...
if (substr_count($x_pieces[0], ':') > 1) {
    exit($x_pieces[0].' has too many colons in the dimension paramter! There should be 1 at most.');
}

// Can't calculate a ratio without a height
if (strstr($x_pieces[0], ':') && ! strstr($x_pieces[0], 'x')) {
    exit('To calculate a ratio a height is needed.');
}
...
switch ($file_format) {
    ...
    case 'webp':
        if (! function_exists('imagewebp')) {
            exit($file_format.' is not supported!');
        }
        imagewebp($img);
        break;
    case 'jpg':
